...
class gdrts_admin_postback {
    ...
    private function transfer() {
        ...
        } else if ($action == 'kk-star-ratings') {
            $message = 'transfer-failed';

            if (isset($post['transfer']['kk-star-ratings'])) {
                require_once(GDRTS_PATH.'core/transfer/kk-star-ratings.php');

                $max = intval($post['transfer']['kk-star-ratings']['max']);

                if ($max > 0) {
                    $message = 'transfered';

                    $obj = new gdrts_transfer_kk_star_ratings();
                    $obj->transfer($max);
                }
            }
