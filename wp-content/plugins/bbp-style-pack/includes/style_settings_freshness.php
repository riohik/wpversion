...
function bsp_style_settings_freshness () {
	...
	<tr>
	<td>
	<p><?php _e('This section allows you to amend freshness', 'bbp-style-pack'); ?> </p>
	</td>
	<td>	
	<?php
	//show style image
	 echo '<img src="' . plugins_url( 'images/freshness_display.JPG',dirname(__FILE__)  ) . '" > '; ?>
	</td>
	</tr>
	</table>
	...
	<!-- checkbox to activate  -->
					...
					<tr valign="top">  
					<th><?php _e('Activate Freshness display', 'bbp-private-groups'); ?></th>
					<td>
					<?php 
					$item = (!empty( $bsp_style_settings_freshness['activate'] ) ?  $bsp_style_settings_freshness['activate'] : '');
	...
	<!-------------------------------Heading---------------------------------------->
					...
					<th>1. <?php _e('Heading Name', 'bbp-style-pack'); ?></th>
					<td colspan="2">
					<?php 
					...
					$item1 = (!empty ($bsp_style_settings_freshness['heading_name'] ) ? $bsp_style_settings_freshness['heading_name']  : '' ) ?>
						...
						<label class="description" for="rlp_settings[heading_label]"><?php _e( 'Default : Freshness - if you wish to change enter the heading description eg "Last Post", "Last updated", "Freshness" "Last activity" etc.', 'bbp-style-pack' ); ?></label><br/>
					</td>
					</tr>
	...
	2.
	<?php _e('Topic Title', 'bbp-style-pack'); ?> </th>
	<td>
	<?php
	...
	<th>
	3. 
	<?php _e('Topic Freshness', 'bbp-style-pack'); ?> </th>
	<td>
	<?php
			...
			if ($check == 1) $item1 = 1 ;
			echo '<input name="'.$item.'" id="'.$item.'" type="checkbox" value="1" class="code" ' . checked( 1,$item1, false ) . ' />' ;
			_e('Click to show Freshness', 'bbp-style-pack');
  			?>
	</td>
	</tr>
	<tr>
	<th>
	4.
	<?php _e('Topic Author name', 'bbp-style-pack'); ?> </th>
	<td>
	<?php
	...
	<th>
	5.
	<?php _e('Topic Author avatar', 'bbp-style-pack'); ?> </th>
	<td>
	<?php
	...
	<th>
	6.
	<?php _e('Freshness format', 'bbp-style-pack'); ?> </th>
	<td>
	...
	<?php
			...
			<br><label class="description"><?php _e( '<i>Default</i>' , 'bbp-style-pack' ); ?></label>
	...
	<td></td><td></td><td>
					...
					$item1 = (!empty ($bsp_style_settings_freshness['date_separator'] ) ? $bsp_style_settings_freshness['date_separator']  : ' at ' ) ?>
						...
						<label class="description" for="bsp_style_settings_freshness[date_separator]"><?php _e( 'eg " at " "," ":" - do not forget to include any spaces needed', 'bbp-style-pack' ); ?></label><br/>
					</td>
					</tr>
...
<?php
	...
	*/
	...
	echo '<label><input type="radio" name="bsp_style_settings_freshness[bsp_date_format]" id="date_format_custom_radio" value="\c\u\s\t\o\m"';
	checked( $custom );
	echo '/> <span class="date-time-text date-time-custom-text">' . __( 'Custom:' ) . '<span class="screen-reader-text"> ' . __( 'enter a custom date format in the following field' ) . '</span></label>' .
		...
		'<input type="text" name="bsp_style_settings_freshness[bsp_date_format_custom]" id="date_format_custom" value="' . esc_attr( get_option( 'date_format' ) ) . '" class="small-text" /></span>' .
...
<?php
	...
	*/
	...
	echo '<label><input type="radio" name="bsp_style_settings_freshness[bsp_time_format]" id="time_format_custom_radio" value="\c\u\s\t\o\m"';
	checked( $custom );
	echo '/> <span class="date-time-text date-time-custom-text">' . __( 'Custom:' ) . '<span class="screen-reader-text"> ' . __( 'enter a custom time format in the following field' ) . '</span></label>' .
		...
		'<input type="text" name="bsp_style_settings_freshness[bsp_time_format_custom]" value="' . esc_attr( get_option( 'time_format' ) ) . '" class="small-text" /></span>' .
